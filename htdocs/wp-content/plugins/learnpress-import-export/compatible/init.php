...
class LPR_Export_Import{
    ...
    function __construct(){
        ...
        $this->_plugin_url 	= untrailingslashit( plugins_url( '/', LPR_EXPORT_IMPORT_FILE ) );
        ...
        if( 'lpr_course' == $post_type ) {
            ...
            <script type="text/javascript">
                jQuery(document).ready(function($) {
                    $('<option>').val('export').text('<?php _e('Export Courses', 'learnpress' )?>').appendTo("select[name='action']");
                    $('<option>').val('export').text('<?php _e('Export Courses', 'learnpress' )?>').appendTo("select[name='action2']");
                });
            </script>
        <?php
        }
    ...
    function course_row_actions( $actions ){
        ...
        if( 'lpr_course' == $post->post_type ){
            ...
            $actions[] = array(
                ...
                'title'     => __( 'Export this course', 'learnpress' ),
                'class'     => 'lpr-export'
            );
        }
        return $actions;
    ...
    function import_upload_form(){
        ...
        <div id="lpr-import-upload-form">
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <br />
                <button class="button"><?php _e( 'Import', 'learnpress' );?></button>
            </form>
        </div>
    <?php
    }
        ...
        if( !$message ){
            ...
            switch ($message) {
                ...
                case 1: // import success with out any duplicate course
                    $type = "updated";
                    $message_text = __('Imports all courses successfully', 'learnpress');
                    break;
                case 2: // import success with some of duplicate course
                    $type = "error";
                    $message_text = __('Some courses are duplicate, please select it in the list to duplicate if you want', 'learnpress');
                    break;

                default: // no course imported
                    $type = "error";
                    $message_text = __('No course is imported. Please try again', 'learnpress');
                    break;
