<?php echo "\n";?><item>
	...
	<?php lpie_export_attachment( $post );?>
	<?php wxr_post_taxonomy(); ?>
	<?php
	if( !in_array( $post->post_author, $options['exported']['authors'] )){
		$options['exported']['author'][] = $post->post_author;
	}
	$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->postmeta WHERE post_id = %d", $post->ID ) );
	foreach ( $postmeta as $meta ) :
		if ( apply_filters( 'wxr_export_skip_postmeta', false, $meta->meta_key, $meta ) )
			continue;
		do_action( 'lp_export_postmeta', $meta->meta_key, $meta );
				...
				if( is_array( $metavalue ) ){
					foreach( $metavalue as $section ) {
						if( !empty( $section['lesson_quiz'] ) ){
							$options['exported']['sections'] = array_merge( $options['exported']['sections'] , $section['lesson_quiz'] );
						}
