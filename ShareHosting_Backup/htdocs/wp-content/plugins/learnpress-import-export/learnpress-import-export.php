...
define( 'LP_IMPORT_EXPORT_VER', '2.0.3' );
...
class LP_Addon_Import_Export_Courses {
	...
	private function _dispatch_export() {
		if ( !empty( $_REQUEST['export-nonce'] ) && wp_verify_nonce( $_REQUEST['export-nonce'], 'learnpress-import-export-export' ) ) {
			if ( empty( $_REQUEST['exporter'] ) ) {
				learn_press_add_notice( __( '<p>Please select a source to export.</p>', 'learnpress' ), 'error' );
			} else {
		...
		if ( !empty( $_REQUEST['import-nonce'] ) && wp_verify_nonce( $_REQUEST['import-nonce'], 'learnpress-import-export-import' ) ) {
			...
			if ( empty( $_REQUEST['import-file'] ) ) {
				...
				if ( !empty( $_REQUEST['import-file'] ) ) {
				    ...
				    if (isset($_REQUEST['step']) and $_REQUEST['step'] == 2) return;
	...
	public function admin_notice() {
		?>
		<div class="error">
			<p><?php printf( __( '<strong>Import/Export Courses</strong> addon version %s requires <strong>LearnPress</strong> version %s or higher', 'learnpress' ), LP_IMPORT_EXPORT_VER, LP_IMPORT_EXPORT_REQUIRE_VER ); ?></p>
		</div>
		<?php
	}
